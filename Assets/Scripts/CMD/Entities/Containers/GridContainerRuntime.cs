...
using UnityEngine;
using UnityEngine.UI; // если будешь класть в UI (не обязательно)

namespace CMD.Entities
{
    [DisallowMultipleComponent]
    public sealed class GridContainerRuntime : MonoBehaviour, IEntityContainer
    {
        ...
        public string ContainerKey => definition ? definition.containerKey : "Backpack";
        ...
        public bool Move(BaseEntityRuntime e, int newIndex, string newSlotKey = null)
        {
            ...
            OnItemMoved?.Invoke(e, i, newIndex);
            return true;
        }
        ...
        // События (можно в IEventBus)
        public event Action<BaseEntityRuntime,int> OnItemAdded;
        public event Action<BaseEntityRuntime,int> OnItemRemoved;
        public event Action<BaseEntityRuntime,int,int> OnItemMoved;
